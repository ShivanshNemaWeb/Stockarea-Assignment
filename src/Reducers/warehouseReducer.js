...
case 'EDIT_WAREHOUSE':
  const index = state.warehouses.findIndex(
    (warehouse) => warehouse.id === state.selectedWarehouse.id
  );

  if (index !== -1) {
    const updatedWarehouses = [...state.warehouses];
    updatedWarehouses[index] = action.payload;
  ...
  return state; // Return the current state if the selectedWarehouse is not found
